...
export const productsReducer = (state = initialState, action: ProductActions): ProductState => {
    switch (action.type) {
        case ProductActionTypes.Add:
            const productIndex = state.shoppingCart.findIndex((item) => item.product.id === action.payload.product.id);
            if (productIndex < 0) {
            ...
            const modifiedProduct: ShoppingCartItem = {...originalProduct, amount: ++originalProduct.amount}
